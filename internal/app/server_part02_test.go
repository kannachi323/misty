...
func TestValidateProductionEnvironmentRejectsInvalidURLAndPort(t *testing.T) {
	...
	if err := validateProductionEnvironment(); err == nil || !strings.Contains(err.Error(), "https") {
	...
	t.Setenv("PORT", "70000")
	if err := validateProductionEnvironment(); err == nil || !strings.Contains(err.Error(), "PORT") {
	...
	if err := validateProductionEnvironment(); err == nil || !strings.Contains(err.Error(), "different Stripe Price IDs") {
	...
	if err := validateProductionEnvironment(); err == nil || !strings.Contains(err.Error(), "STRIPE_PORTAL_RETURN_URL") {
