...
func TestStripeWebhookPathFromEnv(t *testing.T) {
	...
	path, err := stripeWebhookPathFromEnv()
	if err != nil {
		t.Fatalf("stripeWebhookPathFromEnv() default error = %v", err)
	}
	if path != defaultStripeWebhookPath {
		t.Fatalf("default webhook path = %q, want %q", path, defaultStripeWebhookPath)
	}
	...
	for _, invalid := range []string{
		...
		if _, err := stripeWebhookPathFromEnv(); err == nil {
...
func TestMountHandlersUsesConfiguredStripeWebhookPath(t *testing.T) {
	...
	defaultRequest := httptest.NewRequest(http.MethodPost, defaultStripeWebhookPath, strings.NewReader("{}"))
...
func TestValidateProductionEnvironmentRequiresCoreConfiguration(t *testing.T) {
	...
	if err := validateProductionEnvironment(); err == nil || !strings.Contains(err.Error(), "R2_ENDPOINT") {
