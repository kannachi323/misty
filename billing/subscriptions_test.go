...
func TestLoadStripeCheckoutConfig(t *testing.T) {
	...
	if got := cfg.prices[priceKey{db.TierMax, BillingIntervalMonth}]; got != "price_mm" {
...
func TestPricingConstants(t *testing.T) {
	...
	if !validInterval(BillingIntervalMonth) || !validInterval(BillingIntervalYear) || validInterval("week") || validInterval("") {
...
func TestStripeCheckoutConfigRequiresAllFourUniquePrices(t *testing.T) {
	...
	if _, err := loadStripeCheckoutConfig(); err == nil || err.Error() != "STRIPE_PRICE_MAX_YEARLY is required" {
...
func TestCheckoutParamsSelectCatalogPriceAndSetMetadata(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		params := stripeCheckoutSessionParams(cfg, user, testCase.tier, testCase.interval, "", false)
...
func TestOnlyEligibleProCheckoutReceivesTrial(t *testing.T) {
	...
	user := &db.User{ID: "user_test", LicenseID: "license_test", Email: "trial@example.com"}
	pro := stripeCheckoutSessionParams(cfg, user, db.TierPro, BillingIntervalMonth, "", true)
	...
	max := stripeCheckoutSessionParams(cfg, user, db.TierMax, BillingIntervalMonth, "", true)
