...
void ShowFileTree() {
    ...
    // Split the path by '/' and create buttons for each folder
    size_t pos = 0;
    std::string pathSoFar;
    while (pos < currentPath.size()) {
        size_t nextSlash = currentPath.find('/', pos);
        if (nextSlash == std::string::npos) nextSlash = currentPath.size();
        
        std::string folder = currentPath.substr(pos, nextSlash - pos);
        if (!folder.empty()) {
            if (ImGui::Button(folder.c_str())) {
                // Clicking a breadcrumb jumps to that folder
                currentPath = pathSoFar + folder + "/";
            }
            ImGui::SameLine();
            ImGui::Text("/");
            ImGui::SameLine();
    ...
    ImGui::NewLine();
    // Mock contents for demonstration
    ...
    if (currentPath == "/") {
        ...
        folders = {"minidfs"};
    } else if (currentPath == "/Projects/minidfs/") {
